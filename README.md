...
- **EV Charging Layer & Tata EV Simulator**: Toggleable map layer of filtered EV charging stations rendered as custom canvas-drawn glowing lightning bolt symbols. Includes an interactive Tata EV Simulator (for Harrier.ev, Curvv.ev, Nexon.ev, Punch.ev, Tiago.ev, and Tigor.ev) allowing users to input current and target battery charge percentages and view live estimated charging durations calculated per station based on power specs (Fast DC vs. Slow AC) on hover.
